...
```bash
npx skills add dzikrisyairozi/claude-multi-agent-dev            # install selected skills
npx skills add dzikrisyairozi/claude-multi-agent-dev --list     # browse before installing
```

Install a single skill:

```bash
npx skills add dzikrisyairozi/claude-multi-agent-dev --skill spec-driven-development
```
...
> **Note:** Per-skill installs copy only the skill directory itself. If a skill references shared files under `references/`, copy those into the installed skill directory or install the whole pack. See [#361](https://github.com/dzikrisyairozi/claude-multi-agent-dev/issues/361) for background.
...
```bash
git clone https://github.com/dzikrisyairozi/claude-multi-agent-dev.git
```
...
Create an `AGENTS.md` in **your own project** root. This is the system prompt that tells OpenCode when and how to invoke the installed skills. Unlike the repo-scoped `AGENTS.md` in `dzikrisyairozi/claude-multi-agent-dev`, this file belongs to your project and should be adapted to your stack.
...
> **Note:** The root `AGENTS.md` inside the `dzikrisyairozi/claude-multi-agent-dev` repository is intended for contributors working on this repository and should not be copied into other projects. See [CONTRIBUTING.md](../CONTRIBUTING.md#repo-scoped-files).
...
1. Install the skills you need, either with `npx skills add dzikrisyairozi/claude-multi-agent-dev` or by copying them from a clone of this repository into `.opencode/skills/` (project), `~/.config/opencode/skills/` (global), or a cross-compatible path such as `.claude/skills/` / `.agents/skills/`.
...
This keeps the reusable assets (skills) separate from the repository-specific configuration (the `dzikrisyairozi/claude-multi-agent-dev` root `AGENTS.md`).
