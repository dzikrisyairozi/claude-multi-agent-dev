...
```bash
cmd skills add dzikrisyairozi/claude-multi-agent-dev
```
...
```bash
cmd skills add dzikrisyairozi/claude-multi-agent-dev -s spec-driven-development
```

**User scope** (installs into `~/.commandcode/skills/`, available in every project):

```bash
cmd skills add dzikrisyairozi/claude-multi-agent-dev --global
```

**Other supported forms:**

```bash
cmd skills add dzikrisyairozi/claude-multi-agent-dev@main            # a specific branch
cmd skills add dzikrisyairozi/claude-multi-agent-dev/skills/interview-me   # a specific path in the repo
cmd skills add dzikrisyairozi/claude-multi-agent-dev --force         # overwrite / update if already installed
```
